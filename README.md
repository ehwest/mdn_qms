...
This document provides a complete definition of the Quality Management System (QMS) used by Medical Data Networks LLC, and in particular, the QMS system used to deliver T1Pal as an operating internet web software assembly (...as an FDA device).  

More specifically, the present Quality Management System (QMS) is  designed to achieve and sustain the following goals.

1. Meet FDA-defined "General Controls" applicable for the T1Pal Device.
2. Meet FDA-defined "Special Controls" for the T1Pal Device
3. Support "Appropriate Validations" for T1Pal as a device

Now the **"Special Controls"** we refer to has been defined by the FDA as follows:
...
Because T1Pal is a cloud-based server of digital web-site content, the web-site content on the site is the place where all of the special controls are exposed and are the targets of validation.  This would include, for example labeling, user guidance, and warnings.

However, "protections against unauthorized access to and modification of data"  (B1 above) is provided by T1Pal features and functions that are not so readily validated.
For this reason, this QMS includes "Product Definition", "Product Development", and "System Design" sections are needed to convey how this achievement is accomplished and validated.
